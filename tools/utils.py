# -*- coding: utf-8 -*-
# @Author: LlHai
# @Date:   2019-11-28 16:17:08
# @Last Modified by:   LlHai
# @Last Modified time: 2022-04-08 10:33:00
import torch
import cv2
import numpy as np
...
def resize_img(frame, max_length=640):
    ...
    if max(W, H) > max_length:
        ...
        #print("----------resize-------->>",W_resize, H_resize)
...
def draw_3Dimg(plt,fig,pos, image, display=None, kpt2D=None):
...
def videopose_model_load():
    ...
    checkpoint = torch.load(chk_filename, map_location=lambda storage, loc: storage,encoding="utf-8")# 把loc映射到storage
